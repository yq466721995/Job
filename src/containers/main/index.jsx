...
import Cookies from 'js-cookie' //可以从操作前端cookie的对象 set()/get()/remove()
...
class Main extends Component {
    ...
    componentDidMount(){
        //登录过(cookie中有userid)，但没有登录(redux管理中没有_id)，发送请求获取对应的user
    ...
    render() {
        ...
        if(!user._id){
            ...
            //如果有_id,显示对应的界面
            //如果请求根路径，根据user的type和header来计算出一个重定向的路由，并自动重定向
...
1.实现自动登录：
    ...
    2.render()
        ...
        2).判断redux中是否有_id，如果没有，暂时不做任何显示
        3).如果有，说明当前已经登录，显示对应的界面
        4).如果请求根路径:根据user的type和header来计算出一个重定向的路由，并自动重定向
*/